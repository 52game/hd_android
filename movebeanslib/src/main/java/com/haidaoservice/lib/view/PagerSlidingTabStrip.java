...
package com.haidaoservice.lib.view;
...
public class PagerSlidingTabStrip extends HorizontalScrollView implements
    ...
    private boolean allowWidthFull = true; // 内容宽度无法充满时，允许自动调整Item的宽度以充满,默认为true
    ...
    private boolean slidingBlockFull = true; //滑块是否充满
    ...
    private OnNotClickListener notClickListener;

    public void setNotClickListener(OnNotClickListener notClickListener) {
        this.notClickListener = notClickListener;
    }
    ...
    public void setSlidingBlockFull(boolean slidingBlockFull) {
        this.slidingBlockFull = slidingBlockFull;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (tabsLayout == null || tabsLayout.getMeasuredWidth() >= getMeasuredWidth())
            return;
//        if (getTabsLayoutWidth() >= getMeasuredWidth()) {
...
//        }
        ...
        for (View view : views) {
            if (view.getMeasuredWidth() < averageWidth) {
                ...
                if (slidingBlockFull) {
                    ...
                    int margin = (averageWidth - view.getMeasuredWidth()) / 2;
    ...
    public void onClick(View v) {
        if (currentSelectedTabView != v) {
            if (!isClick) {
                if (notClickListener != null) {
                    notClickListener.notClick();
                }
            } else {
     ...
     * 获取布局宽度
     */
    private int getTabsLayoutWidth() {
        int size = getChildCount();
        if (tabsLayout != null && size > 0) {
            int width = 0;
